...
# --- 2. ブラウザストレージによる状態保持（スマホ対策） ---
# JavaScriptを使用してブラウザのLocalStorageに状態を保存・復元する
...
# 初回起動時にLocalStorageから復元
if "initialized" not in st.session_state:
    st.session_state.initialized = True
    # クエリパラメータによる簡易復元（既存仕様）
...
def add_new_row_strictly_at_blank_top(new_data_dict):
    try:
        ...
        no_vals = ws.col_values(1); last_idx = 0
...
# 新規登録
    ...
    st.title("➕ 新規試合登録")
    if st.button("← 戻る"): st.session_state.page = "list"; sync_state_to_storage(); st.rerun()
    with st.form("create_form"):
        ...
        if st.form_submit_button("登録"):
            if add_new_row_strictly_at_blank_top({"カテゴリー": c_cat, "日時": c_date, "競技分類": c_type, "対戦相手": c_opp, "対戦場所": c_loc, "試合分類": c_class, "備考": c_memo}):
                st.session_state.df_list = load_data(); st.session_state.page = "list"; sync_state_to_storage(); st.rerun()

# 一覧画面
else:
    st.title("⚽ KSC試合管理一覧")
    if st.session_state.delete_no:
        st.warning(f"No.{st.session_state.delete_no} を削除しますか？")
        ...
        with cd2:
            if st.button("キャンセル", use_container_width=True): st.session_state.delete_no = None; st.rerun()

    if st.button("➕ 新規試合登録", use_container_width=True):
        st.session_state.page = "create"; sync_state_to_storage(); st.rerun()
    ...
    if not df.empty:
        ...
        edf = st.data_editor(df[disp].reset_index(drop=True), hide_index=True, 
            column_config={"選択": st.column_config.CheckboxColumn("選択", width="small"), "結果入力": st.column_config.CheckboxColumn("結果入力", width="small"), "写真管理": st.column_config.CheckboxColumn("写真管理", width="small"), "日時": st.column_config.DateColumn("日時", format="YYYY-MM-DD"), "No": None}, 
