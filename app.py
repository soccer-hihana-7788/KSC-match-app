...
if st.session_state.selected_year is None:
    ...
    with st.container(border=True):
        ...
        for idx,ご提示いただいたコードに、ダッシュボード（一覧画面）の最上部で**「これまでの総試合数」「競技別の試合数」「カテゴリー別の試合数」を可視化するサマリーパネル**を追加しました。

視覚的に分かりやすくなるよう、Streamlitの `st.metric`（指標表示コンポーネント）を利用し、カード（コンテナ）形式でレイアウトしています。

また、ご提示いただいたコードの中に、Pythonの実行エラーの原因となる特殊な空白（ノーブレークスペース）が多数含まれていたため、**すべて正常な半角スペースに修正**してあります。そのままコピー＆ペーストしてご利用いただけます。
...
def sync_state_to_storage():
    if st.session_state.get("authenticated"):
        ...
        if e_no is not None: st.query_params["e_no"] = str(e_no)
        elif "e_no" in st.query_params: del st.query_params["一覧画面の最上部に、登録されている試合数などを可視化する「ダッシュボード機能」を追加しました。

視覚的にわかりやすくなるよう、Streamlitの `st.metric` コンポーネントを使用し、総試合数・競技別・カテゴリー別の集計をパネルとして配置しています。以下の修正版コードをコピーしてご利用ください。
...
# --- UI: メインダッシュボード（一覧画面） ---
    ...
    if not df.empty:
        with st.container(border=True):
            st.markdown("#### 📈 登録状況サマリー")
            
            # 全体・競技別の集計
            total_games = len(df)
            soccer_games = len(df[df['競技分類'] == 'サッカー'])
            futsal_games = len(df[df['競技分類'] == 'フットサル'])
            
            col1, col2, col3 = st.columns(3)
            col1.metric("⚽ 登録総試合数", f"{total_games} 試合")
            col2.metric("🥅 サッカー", f"{soccer_games} 試合")
            col3.metric("👟 フットサル", f"{futsal_games} 試合")
            
            st.divider()
            
            # カテゴリー別の集計
            st.markdown("##### 🏆 カテゴリー別試合数")
            
            # U8〜U12をベースにしつつ、データ内に存在する独自のカテゴリにも対応
            cat_order = ["U8", "U9", "U10", "U11", "U12"]
            existing_cats = [c for c in df['カテゴリー'].unique() if pd.notna(c) and str(c).strip() != ""]
            ...
            # 実際にデータが存在するカテゴリのみ表示
            display_cats = [c for c in cat_order if c in existing_cats]
            
            if display_cats:
                cat_cols = st.columns(len(display_cats))
                for i, cat in enumerate(display_cats):
                    cat_count = len(df[df['カテゴリー'] == cat])
                    cat_cols[i].metric(f"{cat}", f"{cat_count} 試合")
    # ==========================================
