...
# 結果入力
    ...
    for i in range(1, 11):
        rk = f"res_{no}_{i}"
        curr = all_results.get(rk, {"score": " - ", "scorers": [], "result": ""})
        h_txt = f"第 {i} 試合" + (f" （{curr['result']} {curr['score']}）" if curr['result'] else "")
        with st.expander(h_txt):
            r_opts = ["勝ち", "負け", "引き分け"]
            r_idx = r_opts.index(curr["result"]) if curr["result"] in r_opts else 0
...
# 一覧
    ...
    if st.button("🖨️ 一覧を印刷用表示"):
        if not df.empty:
            ...
            components.html(f"<html><body>{p_df.to_html(index=False)}<script>setTimeout(()=>{{window.print()}},500)</script></body></html>", height=0)
