...
def update_row(actual_index, updated_row_series):
    try:
        ...
        clean_values = [v.item() if isinstance(v, (np.integer, np.floating)) else ("" if pd.isna(v) else v) for v in row_values]
...
if st.session_state.media_no is not None:
    ...
    for i in range(1, 11):
        ...
        with st.expander(f"第 {i} 試合　　{display_score}{result_label}"):
            ...
            with sc_col1:
                new_left = st.text_input("自チーム", value=s_left, key=f"sl_{rk}", label_visibility="collapsed")
            with sc_col2:
                st.markdown("<h3 style='text-align: center; margin: 0;'>-</h3>", unsafe_allow_html=True)
            with sc_col3:
                new_right = st.text_input("相手チーム", value=s_right, key=f"sr_{rk}", label_visibility="collapsed")
