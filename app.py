...
# --- UI: メインダッシュボード（一覧画面） ---
        ...
        with ca2:
            if st.button("コピー", use_container_width=True):
                with st.spinner("コピー中..."):
                    target_row = st.session_state.df_list[st.session_state.df_list["No"] == st.session_state.action_no]
                    if not target_row.empty:
                        r_data = target_row.iloc[0].to_dict()
                        save_data = {
                            "カテゴリー": r_data.get("カテゴリー", ""),
                            "日時": r_data.get("日時", date.today()),
                            "競技分類": r_data.get("競技分類", ""),
                            "対戦相手": r_data.get("対戦相手", "") + " (コピー)",
                            "対戦場所": r_data.get("対戦場所", ""),
                            "試合分類": r_data.get("試合分類", ""),
                            "備考": r_data.get("備考", "")
                        }
                        new_id = update_or_add_row(save_data)
                        if new_id:
                            st.session_state.df_list = load_data()
                            st.session_state.action_no = None
                            sync_state_to_storage()
                            st.success("試合データをコピーしました。")
                            time.sleep(1)
                            st.rerun()
        with ca3:
            if st.button("写真管理", use_container_width=True):
                st.session_state.media_no = st.session_state.action_no
                st.session_state.action_no = None
                sync_state_to_storage()
                st.rerun()
        with ca4:
    ...
    df_show = st.session_state.df_list.copy()
    
    if sq:
        df_show = df_show[
            df_show['対戦相手'].str.contains(sq, na=False) | 
            df_show['対戦場所'].str.contains(sq, na=False) | 
            df_show['備考'].str.contains(sq, na=False)
        ]
    if cf != "すべて":
        df_show = df_show[df_show['カテゴリー'] == cf]

    edited_df = st.data_editor(
        df_show,
        column_config={
            "選択": st.column_config.CheckboxColumn("選択", required=True),
            "試合詳細": st.column_config.CheckboxColumn("試合詳細", required=True),
            "写真管理": st.column_config.CheckboxColumn("写真管理", required=True),
